          ...
          {segments.map((s, i) => {
            ...
            return (
              <button
                ...
                className={clsx(
                  'px-3.5 py-1.5 rounded-sm text-xs font-semibold flex items-center gap-1.5',
                  activeSegment === i ? 'bg-primary text-white' : 'bg-slate-100 text-muted',
                )}
              >
                {s.departure_airport_code} → {s.arrival_airport_code}
                {count > 0 && (
                  <span className={clsx('rounded-sm text-[10px] w-4 h-4 flex items-center justify-center', activeSegment === i ? 'bg-white/25' : 'bg-primary/15 text-primary')}>
                    {count}
                  </span>
                )}
              </button>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {catalog.map((item) => {
              ...
              return (
                <div key={item.ID} className="flex items-center justify-between gap-3 bg-slate-50/70 rounded-md px-4 py-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      disabled={qty === 0}
                      className="w-7 h-7 rounded-sm border border-slate-200 flex items-center justify-center disabled:opacity-30"
                    >
